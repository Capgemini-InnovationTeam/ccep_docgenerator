        ...
        complete: function (jqXHR) {
            ...
            $.when(firstDeferred, secondDeferred, standardDeferred).then(
                function () {
                    ...
                    try {
                        that.Tittle = standardParsed[1]["core_sections"][1]["Title"] || "Technical Specification Document";
                    } catch {
...
callfunction_Report:      function () { return this._makeApiCall("report",      "report");      },
        ...
        createHeadingTSD: function (documentChildren, response) {
            ...
            var core_sections = safeArr(response[1] && response[1]["core_sections"]);
            var title = (core_sections[1] && core_sections[1].Title) ? core_sections[1].Title : "Technical Specification Document";
            ...
            var refItems2 = (core_sections[1] && core_sections[1].ReferenceItems) ? core_sections[1].ReferenceItems : [{}];
            var refItems = safeObj(safeArr(refItems2)[0]);

            // NEW response returns "Incoming References" as a stringified JSON array of
            // destination objects, e.g. [{"destinationPageTitle":"FSD E-0024 ..."}].
            // Parse it and produce a comma-separated list of human-readable page titles
            // so the Word doc matches the old clean format.
            function formatIncomingRefs(raw) {
                if (raw === null || raw === undefined) return "";
                var s = String(raw).trim();
                if (!s) return "";
                var first = s.charAt(0);
                if (first !== "[" && first !== "{") return s; // Already a plain string
                try {
                    var arr = JSON.parse(s);
                    if (!Array.isArray(arr)) arr = [arr];
                    var labels = arr.map(function (it) {
                        var o = (it && typeof it === "object") ? it : {};
                        return o.destinationPageTitle ||
                               o.destinationSpaceTitle ||
                               o.destinationSpaceKey ||
                               o.urlPath || "";
                    }).filter(function (l) { return String(l).trim() !== ""; });
                    return labels.length ? labels.join(", ") : s;
                } catch { return s; }
            }

            var refData = [
                { "Reference Type": "Incoming References", "Value": formatIncomingRefs(refItems["Incoming References"]) },
            ...
            var relItemsData = safeObj(safeArr((core_sections[1] && core_sections[1].RelatedItems) ? core_sections[1].RelatedItems : [{}])[0]);
            var relData = [
            ...
            var cls = safeObj(safeArr((core_sections[1] && core_sections[1].classification) ? core_sections[1].classification : [{}])[0]);
            var clsKVs = [
        ...
        createInitialCommonSectionTSD: function (documentChildren, response) {
            ...
            var core_sections = safeArr(response[1] && response[1]["core_sections"]);
            // NEW response format: additional_sections lives at response[2].additional_sections
            // OLD response format: response3 lived at response[1].response3
            var response3 = safeObj(
                (response[2] && response[2]["additional_sections"])
                    ? response[2]["additional_sections"]
                    : (response[1] && response[1]["response3"])
            );
            ...
            safeSection(D, documentChildren, "Brief Description", function () {
                ...
                var briefDescription = (core_sections[0] && core_sections[0]["brief_description"]) ? core_sections[0]["brief_description"] : "";
                if (briefDescription) {
            ...
            safeSection(D, documentChildren, "Bespoke Development Details", function () {
                ...
                var bespoke = safeObj(safeArr((core_sections[0] && core_sections[0].BespokeDevelopmentDetails) ? core_sections[0].BespokeDevelopmentDetails : [{}])[0]);
            ...
            safeSection(D, documentChildren, "Data Descriptions", function () {
                ...
                // core_sections[2] may be { Data_description: [ {...} ] }
                var dataDescWrapper = core_sections[2] && core_sections[2]["Data_description"]
                    ? safeArr(core_sections[2]["Data_description"])
                    : [];
                ...
                // NEW response uses PascalCase key "UpdateMethodForTables"; OLD used "Update method for tables"
                var updMethodVal = dataDesc["UpdateMethodForTables"] || dataDesc["Update method for tables"] || "";
                documentChildren.push(new Paragraph({ children: [new TextRun({ text: updMethodVal, size: 20, font: "Helvetica" })], spacing: { before: 100, after: 200 } }));
            });
        ...
        createLastCommonSectionTSD: function (documentChildren, response) {
            ...
            // NEW response wraps TechnicalUnitTestDetails under a "test_details" object at response[0]
            // OLD response had it directly at response[0].TechnicalUnitTestDetails
            var techBlock =
                (response[0] && response[0]["test_details"] && response[0]["test_details"]["TechnicalUnitTestDetails"])
                    ? response[0]["test_details"]["TechnicalUnitTestDetails"]
                    : (response[0] && response[0]["TechnicalUnitTestDetails"]);
            var TECHNICALTESTING = safeObj(safeArr(techBlock)[0]);

            var core_sections = safeArr(response[1] && response[1]["core_sections"]);
            // NEW response moved these keys to response[2].additional_sections
            // OLD response had them at response[1].response3
            var response3 = safeObj(
                (response[2] && response[2]["additional_sections"])
                    ? response[2]["additional_sections"]
                    : (response[1] && response[1]["response3"])
            ...
            safeSection(D, documentChildren, "Technical Unit Test Details", function () {
                ...
                // NEW response provides an array under "Test tools and environment"
                var ttEnv = safeArr(TECHNICALTESTING["Test tools and environment"]);
                var ttFirst = safeObj(ttEnv[0]);
                var ttRows = [
                    makeKVRow(D, "Test System", ttFirst["Test System"] || ""),
                    makeKVRow(D, "Client", ttFirst["Client"] || ""),
                    makeKVRow(D, "Tool", ttFirst["Tool"] || "( ) CATT\n( ) Workbench Debugger\n( ) Other"),
                    makeKVRow(D, "Test program(s)", ttFirst["Test Program(s)"] || ttFirst["Test Program"] || "")
                ];
                documentChildren.push(new Table({ rows: ttRows }));

                // Testing Scope (NEW field in response)
                var scope = safeObj(TECHNICALTESTING["Testing scope"]);
                if (scope["Within"] || scope["Outside"]) {
                    documentChildren.push(new Paragraph({ children: [new Bookmark({ id: "TestingScope", children: [new TextRun({ text: "Testing Scope", bold: true, size: 24, font: "Helvetica" })] })], spacing: { before: 250, after: 100 } }));
                    documentChildren.push(new Table({ rows: [
                        makeKVRow(D, "Within", scope["Within"] || ""),
                        makeKVRow(D, "Outside", scope["Outside"] || "")
                    ] }));
                }
                ...
                // NEW response uses "Programmer Email" and "Tester Email" instead of a single "Email"
                var execCols = ["Programmer Name", "Programmer Email", "Tester Name (if different)", "Tester Email", "Program Under Test", "Test Date"];
            ...
            safeSection(D, documentChildren, "Additional Information", function () {
                ...
                if (ooDetails.length) {
                    // NEW response fields use PascalCase (MethodName, MethodDescription, MethodType, MethodDetails)
                    var ooCols = ["Method Name", "Description", "Method Type", "Method Details", "Import/Export/Change/Return Parameters"];
                    var ooMap  = {
                        "Method Name": "MethodName",
                        "Description": "MethodDescription",
                        "Method Type": "MethodType",
                        "Method Details": "MethodDetails",
                        "Import/Export/Change/Return Parameters": "Import/Export/Change/Return Parameters"
                    };
                    // Merge in fallbacks for old response (space-separated keys)
                    var ooRows = ooDetails.map(function (r) {
                        return {
                            "MethodName": r["MethodName"] || r["Method Name"] || "",
                            "MethodDescription": r["MethodDescription"] || r["Description"] || r["ClassDescription"] || "",
                            "MethodType": r["MethodType"] || r["Method Type"] || "",
                            "MethodDetails": r["MethodDetails"] || r["Method Details"] || "",
                            "Import/Export/Change/Return Parameters": r["Import/Export/Change/Return Parameters"] || ""
                        };
                    });
                    documentChildren.push(makeColumnTable(D, ooCols, ooRows, ooMap));
                } else {
                ...
                if (bgDetails.length) {
                    ...
                    // NEW response has JobCatalogDescription + JobTemplateDescription instead of a single Description
                    // Merge them into the Description column so the field renders instead of showing blank
                    var bgRows = bgDetails.map(function (b) {
                        var desc = b["Description"];
                        if (!desc) {
                            var parts = [];
                            if (b["JobCatalogDescription"]) { parts.push("Job Catalog: " + b["JobCatalogDescription"]); }
                            if (b["JobTemplateDescription"]) { parts.push("Job Template: " + b["JobTemplateDescription"]); }
                            desc = parts.join("\n");
                        }
                        return {
                            "JobCatalog": b["JobCatalog"] || "",
                            "JobCatalogEntry": b["JobCatalogEntry"] || "",
                            "JobTemplate": b["JobTemplate"] || "",
                            "Description": desc || "",
                            "System": b["System"] || "",
                            "Class": b["Class"] || "",
                            "ExecuteMethod": b["ExecuteMethod"] || "",
                            "GlobalLocal": b["GlobalLocal"] || b["Global/Local"] || "",
                            "BusinessUnitCountry": b["BusinessUnitCountry"] || b["Business Unit/Country"] || "",
                            "LocalizationReason": b["LocalizationReason"] || b["Localization Reason"] || "",
                            "TargetIntermediateState": b["TargetIntermediateState"] || b["Target/Intermediate State"] || "",
                            "RelatedJIRAIssues": b["RelatedJIRAIssues"] || b["Related JIRA Issues"] || "",
                            "AdditionalInformation": b["AdditionalInformation"] || b["Additional Information"] || ""
                        };
                    });
                    ...
                    documentChildren.push(makeColumnTable(D, bgCols, bgRows, bgMap));
                } else {
            ...
            safeSection(D, documentChildren, "Assumptions", function () {
                ...
                var assumptions = (core_sections[1] && core_sections[1]["Assumptions"])
                    ? (Array.isArray(core_sections[1]["Assumptions"]) ? core_sections[1]["Assumptions"].join("\n") : core_sections[1]["Assumptions"])
                    : "";
            ...
            safeSection(D, documentChildren, "Authorization Objects", function () {
                ...
                if (authObjs.length) {
                    // NEW response provides: Authorization Object / Type / Additional Information
                    // OLD response had: Authorization Object / Restriction / Fields / Roles
                    var authCols = ["Authorization Object", "Type", "Additional Information"];
                    var authMap = {
            ...
            safeSection(D, documentChildren, "Dependencies", function () {
                ...
                var deps = (core_sections[0] && core_sections[0].Dependencies) ? core_sections[0].Dependencies : "";
    ...
    documentChildren.push(new Paragraph({
        children: [new TextRun({
            ...
            safeSection(D, documentChildren, "Enhancement - Implementing Class Details", function () {
                ...
                // NEW response uses ClassDescription + MethodDetails (no Method_Name/Type/Details keys)
                // Derive Method Name from MethodDetails' first identifier token when not provided.
                var methodNameFromDetails = "";
                if (!icd["Method_Name"] && icd["MethodDetails"]) {
                    var mMatch = String(icd["MethodDetails"]).match(/\b([A-Z][A-Z0-9_]{2,})\b/);
                    if (mMatch) { methodNameFromDetails = mMatch[1]; }
                }
                documentChildren.push(makeColumnTable(D, ["Method Name", "Description", "Type", "Details"],
                    [{
                        "Method Name": icd["Method_Name"] || icd["MethodName"] || methodNameFromDetails || "",
                        "Description": icd["Description"] || icd["ClassDescription"] || "",
                        "Type":        icd["Type"] || icd["MethodType"] || "",
                        "Details":     icd["Details"] || icd["MethodDetails"] || ""
                    }]
                ));
            });
        ...
        _appendCleanCoreRecommendations: function (documentChildren, response_cleancore, D) {
            ...
            // Helper: flatten a value that may be an object with numeric-string keys
            // (new response format: {"1": "text A", "2": "text B"}) into an array
            // of its values, ordered by numeric key when possible.
            function flattenRecValue(val) {
                if (val === null || val === undefined) return "";
                if (Array.isArray(val)) return val;
                if (typeof val === "object") {
                    var subKeys = Object.keys(val);
                    if (!subKeys.length) return "";
                    // Sort numerically if all keys look like numbers
                    var allNumeric = subKeys.every(function (k) { return /^\d+$/.test(k); });
                    if (allNumeric) {
                        subKeys.sort(function (a, b) { return parseInt(a, 10) - parseInt(b, 10); });
                    }
                    return subKeys.map(function (k) {
                        var v = val[k];
                        // Recurse one level for nested objects (rare) — otherwise stringify
                        if (v && typeof v === "object" && !Array.isArray(v)) {
                            return Object.keys(v).map(function (kk) { return v[kk]; }).join(" ");
                        }
                        return String(v || "");
                    }).filter(function (s) { return s.trim() !== ""; });
                }
            ...
            recs.forEach(function (rec) {
                ...
                keys.forEach(function (key) {
                    // NEW response nests recommendation text inside a { "1": "...", "2": "..." } object
                    // Flatten these so pushRow receives a plain array/string, not "[object Object]"
                    pushRow(key, flattenRecValue(recObj[key]));
                });
            });
...
createWorkflowTSD: function (documentChildren, response, response_cleancore) {
    ...
    var oTsdNudgeBox = this.byId("tsdNudgeBox");
    if (oTsdNudgeBox) { oTsdNudgeBox.setVisible(true); }

    var oStep1 = this.byId("id_TSDStep1");
